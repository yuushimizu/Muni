        ...
        var drawCell = function(cell) {
            ...
            if (cell.vitality.current > 0) {
                ...
                var adjustColor = function(source) {
                    var rated = Math.floor(512 * 3 * source / rgbRateTotal);
                    return rated < 0 ? 0 : (rated > 255 ? 255 : rated);
                };
                var ratedColor = {
                    red: adjustColor(cell.rgbRate.red),
                    green: adjustColor(cell.rgbRate.green),
                    blue: adjustColor(cell.rgbRate.blue)};
                var gradient = context.createRadialGradient(cell.x, cell.y, radius * 0.5, cell.x, cell.y, radius * 1.5);
                gradient.addColorStop(0, 'rgba(' + ratedColor.red + ',' + ratedColor.green + ',' + ratedColor.blue + ',' + 1 + ')');
                gradient.addColorStop(1, 'rgba(' + ratedColor.red + ',' + ratedColor.green + ',' + ratedColor.blue + ',' + 0 + ')');
                context.fillStyle = gradient;
                context.arc(cell.x, cell.y, radius * 1.5, 0, Math.PI * 2, false);
                context.closePath();
                context.fill();
                context.beginPath();
