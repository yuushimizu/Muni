...
- (void)sendFrameWithCell:(id<MNCell>)cell withEnvironment:(id<MNEnvironment>)environment {
	...
	if (scanningResults.count > 0) {
		...
		for (MNCellScanningResult *scanningResult in scanningResults) {
			CGPoint pointWithMaxDistance = JZMovedPoint(cell.center, JZRadianFromPoints(cell.center, scanningResult.cell.center), maxDistance);
		...
		if (cellX > environment.field.size.width / 2) {
			...
			if (distanceToWall < maxDistance / 2) xDestination = cellX;//-= maxDistance / 2 - distanceToWall;
		} else {
			if (cellX < maxDistance / 2) xDestination = cellX;//+= maxDistance / 2 - cellX;
		}
		if (cellY > environment.field.size.height / 2) {
			double distanceToWall = environment.field.size.height - cellY;
			if (distanceToWall < maxDistance / 2) yDestination = cellY;//-= maxDistance / 2 - distanceToWall;
		} else {
			if (cellY < maxDistance / 2) yDestination = cellY;//+= maxDistance / 2 - cellY;
		}
		double radian;
		if (xDestination == cellX && yDestination == cellY) {
			radian  = MNRandomRadian();
		} else {
			radian = JZRadianFromPoints(cell.center, CGPointMake(xDestination, yDestination));
		}
		[cell moveFor:radian];
