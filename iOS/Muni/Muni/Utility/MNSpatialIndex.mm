...
- (id)initWithBlockSize:(const juiz::Size &)blockSize withBlockCount:(const juiz::Size &)blockCount {
	if (self = [super init]) {
		...
		const int blockCountX = _blockCount.width();
		const int blockCountY = _blockCount.height();
...
- (id)initWithTotalSize:(const juiz::Size &)totalSize withBlockCount:(const juiz::Size &)blockCount {
	return [self initWithBlockSize:juiz::Size(totalSize.width() / blockCount.width(), totalSize.height() / blockCount.height()) withBlockCount:blockCount];
}
